...
public extension State where Event: MappableEvent {
    
    func and<T: MappableEvent>(_ state: @autoclosure @escaping Supplier<State<T>>) -> State<Event.Event> where T.Event == Event.Event {
...
public extension State {
    ...
    func and<C: MappableEvent>(_ state: @autoclosure @escaping Supplier<State<C>>) -> State<Event> where C.Event == Event {
        state().and(self)
    }
