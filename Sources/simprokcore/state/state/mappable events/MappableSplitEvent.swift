...
public extension State where Event: MappableEvent {
    ...
    func or<T: MappableEvent>(_ state: @autoclosure @escaping Supplier<State<T>>) -> State<Event.Event> where T.Event == Event.Event {
...
public extension State {
    ...
    func or<C: MappableEvent>(_ state: @autoclosure @escaping Supplier<State<C>>) -> State<Event> where C.Event == Event {
        state().or(self)
    }
